 ...
 *
 * Enabled languages come from the in-app language picker when set; otherwise Teclas uses just the
 * phone's PRIMARY system language (intersected with the lists we ship). Multilingual typing is an
 * explicit opt-in — the user picks the extra languages in Teclas settings — so a secondary phone
...
object DictionaryLoader {
    ...
    // In-app language selection (comma-separated codes), stored in the shared "teclas" prefs. When
    ...
    fun enabledLanguages(context: Context): List<String> {
        ...
        // English silently "corrected" into Spanish they never asked Teclas for. Multilingual typing

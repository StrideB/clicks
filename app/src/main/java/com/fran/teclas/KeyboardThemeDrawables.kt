...
object KeyboardThemeDrawables {
    ...
    val addedThemes = listOf(GOOGLE, IOS, PIXEL_SAND, TECLAS_GLASS)

    fun isAddedTheme(theme: String): Boolean = theme in addedThemes

    fun isLight(theme: String, darkMode: Boolean): Boolean = when (theme) {
        GOOGLE, IOS -> !darkMode
        PIXEL_SAND -> true
        TECLAS_GLASS -> false
        else -> !darkMode

...
object AgenticEngine {
    ...
    private fun aiReady(host: AgenticHost): Boolean {
        ...
        if (!GeminiClient.configured(host.prefs())) { host.flashStatus("Add a Gemini key in Clicks settings", 2400); return false }
        return true
    }
    ...
    private fun stock(host: AgenticHost, ticker: String) {
        ...
        if (key.isBlank()) { host.flashStatus("Add a Finnhub key in Clicks settings", 2600); return }
    ...
    private fun odds(host: AgenticHost) {
        ...
        if (key.isBlank()) { host.flashStatus("Add an odds API key in Clicks settings", 2600); return }
    ...
    private fun meet(host: AgenticHost) {
        if (!host.isGoogleConnected()) { host.flashStatus("Connect Google in the Clicks app first", 2800); return }
    ...
    private fun notion(host: AgenticHost, query: String) {
        ...
        if (token.isBlank()) { host.flashStatus("Add a Notion token in Clicks settings", 2800); return }

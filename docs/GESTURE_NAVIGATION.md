...
There *is* a way around it on Xiaomi, and it does not need another app installed. Where there is
not, the launcher now ships its own gesture navigation.
...
`1` keeps full-screen gestures available with any default home. That is what step 1 writes.
...
### 1 · Ask the system for its own gestures

Best outcome by a distance: real animations, real quick-switch, nothing intercepting touches.
...
### 2 · The launcher's own gesture bar

The fallback that works everywhere, including ROMs that will not hand their gestures back.
...
apps keep theirs. Only offered when step 2 is on, so hiding the buttons never leaves the home screen
with no way off it.
...
notification shade, and it clears itself at the next reboot. Only worth it with step 1 or step 2
